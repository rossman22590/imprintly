...
// Defaults: $0.01 per credit, 20% audiobook markup.
//   Eleven v3 / Multilingual v2: $0.10 / 1000 chars -> 0.10 * 1.2 / 0.01 = 12 credits / 1k
//   Flash / Turbo:               $0.05 / 1000 chars -> 0.05 * 1.2 / 0.01 = 6 credits / 1k

test("eleven v3 costs 12 credits per 1,000 characters", () => {
...
test("multilingual v2 costs 12 credits per 1,000 characters", () => {
  const charge = calculateAudioCharge({
...
test("flash/turbo cost 6 credits per 1,000 characters", () => {
  assert.equal(
...
test("charge scales linearly with character count", () => {
  ...
  assert.equal(charge.credits, 600);
});
